...
export default function SerieDetails() {
    ...
    const getItems = async () => {
        ...
        const data = await fetch(`${API_ROUTE}/tv/${id}?language=pt-BR&api_key=${API_KEY_AUTH}`,
            JSON.parse(OPTIONS_FETCH))
            .then(response => response.json())
            .then(data => data)
            .catch(err => console.log(err))
        const recommendations = await fetch(`${API_ROUTE}/tv/${id}/recommendations?language=pt-BR&page=1&api_key=${API_KEY_AUTH}`,
            JSON.parse(OPTIONS_FETCH))
            .then(response => response.json())
            .then(data => data)
            .catch(err => console.log(err))
                ...
                <main className={`bg-bgd flex min-h-screen flex-col`}>
                   {/*  {itens?.data ?
                        <><section className={`w-full h-fit relative bg-center bg-cover flex`}
                            ...
                            <img className='lg:h-96 md:h-64 h-60 w-fit rounded-xl relative md:my-10 my-5 md:ml-10 mx-auto' src={`https://www.themoviedb.org/t/p/w600_and_h900_bestv2${itens.data.poster_path}`}
                            ...
                            <section className='p-4'>
                                ...
                                <div className='flex gap-4 overflow-x-scroll py-4 scroll-custom'>
                                    {itens.recommendations.length > 0 ?
                                        itens.recommendations.map((item, key: number) => (<RecommendationItem type='serie' key={key} item={item} />))
